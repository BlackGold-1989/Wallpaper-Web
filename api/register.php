...
if(!empty($username) && !empty($email) && !empty($password) && !empty($device_type) && !empty($device_token)) {
    try
    {
        ...
        $stm = $db->prepare("INSERT INTO status_users (username,email,password,profile_image,device_type,device_token) VALUES (:username, :email, :password, :profile_image, :device_type, :device_token)") ;
        // inserting a record
        $stm->execute(array(':username' => $username , ':email' => $email , ':password' => $password
         , ':profile_image' => $profile_image,'device_type' => $device_type, 'device_token' => $device_token));
